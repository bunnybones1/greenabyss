...
var Clouds = require('./SpriteClouds');
...
function Sky(params) {
	...
	atmosphereColor = new THREE.Color(params.skyColor);
	...
	this.samplesGroundColor = emptyColors;
	...
	function onMapLoad(texture) {
		...
		var samplesGroundColor = [];
		for (var i = 0; i < params.mapFrames; i++) {
			samplesSkyColor.push(sampleImage(1, i * params.mapFrameHeight + 6));
			samplesSunColor.push(sampleImage(2, i * params.mapFrameHeight + 6));
			var atmoColor = sampleImage(4, i * params.mapFrameHeight + 2);
			atmoColor.lerp(sampleImage(4, i * params.mapFrameHeight + 3), 0.25);
			samplesAtmosphereColor.push(atmoColor);
			samplesGroundColor.push(sampleImage(0, i * params.mapFrameHeight + 6));
		}
		...
		_this.samplesGroundColor = samplesGroundColor;
	}
	...
	var clouds = new Clouds({
		...
		mapFrames: params.mapFrames, 
		mapTotalHeight: params.mapTotalHeight,
		height: params.radius * 0.05,
		radius: params.radius,
		total: 50,
		sizeMin: 0.02,
		sizeMax: 0.05,
		layer: params.layer,
		colorFog: atmosphereColor,
	...
	sky.layers.set(params.layer);
	stars.layers.set(params.layer);
	sun.layers.set(params.layer);
	...
	this.shadowOffset = new THREE.Vector3(10, 0, 0);
}
...
Sky.prototype.update = function() {
	...
	this.clouds.material.progress = skyProgress;
	...
	// this.sunLight.position.add(this.shadowOffset);
	// this.sunLight.target.position.copy(this.shadowOffset);
	// this.sunLight.shadow.camera.top = 10 - this.shadowOffset.x;
	// this.sunLight.shadow.camera.bottom = -10 - this.shadowOffset.x;
	// this.sunLight.shadow.camera.left = -10;
	// this.sunLight.shadow.camera.right = 10;


	var remainder = skyProgress % this.mapFramePercentOfDay;
	var frame1 = Math.round((skyProgress - remainder) * this.mapFrames + 1) % this.mapFrames;
	...
	this.groundColor.copy(this.samplesGroundColor[frame1]);
	this.groundColor.lerp(this.samplesGroundColor[frame2], mix);
	this.atmosphereColor.copy(this.samplesAtmosphereColor[frame1]);
	this.atmosphereColor.lerp(this.samplesAtmosphereColor[frame2], mix);
