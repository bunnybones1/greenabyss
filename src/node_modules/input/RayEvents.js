...
const isMobile = require('utils/devices/isMobile');
const isVR = require('utils/devices/isVR');
...
function RayEvents (element) {
  ...
  if(element.clientWidth === undefined) {
    throw new Error('Ray Events are designed to be in clipspace, for quick use in webGL. The element you\'re listening to doesn\'t have a size, so we can\'t use it to calculate clip coordinates.' +
      'Also just in case clientWidth isn\'t available, width needs to be pre-pixelDeviceRatio, so canvas.width does not work.');
  }
...
function _onStart(ev, pos) {
  var nope = pos[0] === 0 && pos[1] === 0;
  _toClipSpace.call(this, pos);
  if(!nope) this.dispatchOnMove(pos);
  ev.preventDefault();
...
function _onEnd(ev, pos) {
  ...
  if(!nope) this.dispatchOnMove(pos);
  ev.preventDefault();
  const ms = performance.now() - this.lastTime;
