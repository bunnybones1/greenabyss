...
function DynamicTextureManager(params) {
	...
	for (var i = 0; i < 16; i++) {
		var radius = (i * 0.666 + 0.5);
		var ringGeom = new THREE.TorusGeometry(radius, 0.15, 16, 8);
...
function morphUvsToMaterialSwatch(geometry, alias) {
	...
	if(geometry.attributes) {
		...
		var uvFaceArray = geometry.faceVertexUvs[0];
		for (var i = 0; i < uvFaceArray.length; i++) {
			var uvArray = uvFaceArray[i];
			for (var j = 0; j < uvArray.length; j++) {
				var uv = uvArray[j];
				uv.x = (uv.x * paddedScale + col + padding) / cols;
				uv.y = (uv.y * paddedScale + row + padding) / rows;
			}
		}
	} else {
		throw new Error('Where are the uvs?');
	}
}
